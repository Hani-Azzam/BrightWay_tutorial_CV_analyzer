...
chunk_config = ChunkConfig(
    ...
    separators = ["\n\n", "\n", ". ", " ", "", "**CONTACT**", "**ABOUT**", "**EDUCATION**",
                             "**JOB EXPERIENCE**", "**SKILLS**", "**PROJECTS**"] # Consider tweaking this
)


# cleanup_on_exit=False keeps the Pinecone index alive between runs
store    = DocumentStore(embedder, pc_config, chunk_config, cleanup_on_exit=False)
...
# rag_agent = RagAgent(llm_client, vector_memory, pipeline, store, DATA_CARDS)
...
agent.index_single_cv("data/CVs/CV_01.txt")
print("Done.\n")
